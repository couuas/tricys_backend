...
2. **访问 API 文档**:
   访问 `http://localhost:8000/docs` 查看交互式 Swagger UI。
...
- `api/`: API 路由定义与交互逻辑。
- `core/`: 全局配置与系统事件。
- `models/`: 数据库 Schema 与数据校验模型。
- `services/`: 核心业务逻辑 (引擎管理、队列调度、文件管理、HDF5 读取)。
- `utils/`: 通用工具类 (WebSocket 管理器、逻辑日志)。
